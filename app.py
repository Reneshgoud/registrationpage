...
- Full exception logging so real errors show up in your Render logs
"""
...
# Make sure errors actually show up in Render's log stream (stdout/stderr).
...
def get_db():
    ...
    if db is None:
        # timeout=10 makes SQLite wait (instead of instantly erroring) if
        # another worker process/thread currently has the file locked.
        ...
        # blocking each other — important once Render runs >1 worker.
...
def register():
    ...
    try:
        ...
        # Default method (no hardcoded "pbkdf2:sha256:260000" string) avoids
        # breaking across different installed Werkzeug versions.
        ...
        # Race condition: two requests for the same email at once.
        db.rollback()
        return jsonify(ok=False, errors={"email": "An account with this email already exists."}), 409
    except Exception:
        db.rollback()
...
def server_error(e):
    # Log the real exception so it shows up in Render's Logs tab.
